...
import { faUser, faLocationDot, faCaretRight, faImage, faClose, faCircleXmark, faChevronCircleRight, faCheck, faChevronCircleLeft, faPlus } from "@fortawesome/free-solid-svg-icons"
...
const Request = () => {
    ...
    const validate = (e) =>{
       ...
       // console.log(empty)
        if(empty.length >= 1 && files.length < 1){
            ...
            else if(empty[index] === 'max_price'){
                price_message.current.innerText = "Both prices are required"
                maxprice.current.style.border = '1px solid red'
            }
            else if(empty[index] === 'min_price'){
                price_message.current.innerText = "Both prices are required"
                minprice.current.style.border = '1px solid red'
            }
    ...
    const handleprice =(val, refs, refss, fields)=>{
   ...
   // console.log(bcstore)
    const axe =async (e)=>{
            ...
            data.append('max_price', payload.max_price);
            data.append('min_price', payload.min_price);
            ...
            data.append('featured_image', payload.feat_image);
            ...
            try {
                const response = await axios.post('http://vensle.com/api/api/requests', data);
                                ...
                                <div className="adm-main-second" style={{display:'grid', gridTemplateColumns:'repeat(2, 49%)', gap:'1%'}}>
                                    <div className="main-input">
                                        ...
                                        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'5px'}}>
                                            <div className="input-field" ref={minprice}>
                                                <div style={{display:'grid', gridTemplateColumns:'50px 1fr', margin:'0', alignItems:'center'}}>
                                                    <p>
                                                        {decode(currency)}
                                                    </p>
                                                    <input type='text' placeholder="Minimum" onChange={(e)=>handleprice(e.target.value, minprice, price_message, 'min_price')} ref={priceinput}/>
                                                </div> 
                                            </div> 
                                            <div className="input-field" ref={maxprice}>
                                                <div style={{display:'grid', gridTemplateColumns:'50px 1fr', margin:'0', alignItems:'center'}}>
                                                    <p>
                                                        {decode(currency)}
                                                    </p>
                                                    <input type='text' placeholder="Maximum" onChange={(e)=>handleprice(e.target.value, maxprice, price_message, 'max_price')} ref={priceinput}/>
                                                </div>
                                            </div>
                                        </div> 
                                    </div>
